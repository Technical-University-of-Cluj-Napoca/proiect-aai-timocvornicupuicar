...
def retrieve_context_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    k = 5 + (iteration * 3)
    threshold = max(0.1, 0.3 - (iteration * 0.1))
    
    print(f"Retrieving context for {len(parsed_doc.clauses)} clauses (k={k}, threshold={threshold})...")
    ...
    append_node_log(state["timestamp"], f"retrieve_context (iter {iteration})", duration, tokens_consumed=0)
...
def assess_risk_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    duration = time.time() - start_time
    append_node_log(state["timestamp"], "assess_risk", duration, tokens_consumed=total_tokens)
...
def flag_high_risk_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    duration = time.time() - start_time
    append_node_log(state["timestamp"], "flag_high_risk", duration, tokens_consumed=0)
    ...
    for clause in parsed_doc.clauses:
        ...
        if risk:
            ...
            if risk.risk_level == RiskLevel.MEDIU:
                total_tokens += 1500 # Single LLM call
            elif risk.risk_level == RiskLevel.RIDICAT:
                total_tokens += 6000 # 3+1 self-consistency calls
                
    duration = time.time() - start_time
    append_node_log(state["timestamp"], "generate_recommendations", duration, tokens_consumed=total_tokens)
...
def compile_report_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    report_path = state.get("report_path") or f"data/contract_audit_report_{state['timestamp']}.md"
    ...
    duration = time.time() - start_time
    append_node_log(state["timestamp"], "compile_report", duration, tokens_consumed=0)
...
def quality_check_edge(state: WorkflowState) -> str:
    """
    Decides whether to retry retrieval if there are too many NECUNOSCUT (unknown) risk claues.
    If unknown count > 40% and iteration < 2, go back to retrieve_context with more permissive parameters.
    """
    ...
    print(f"[Quality Check] Unknown risk ratio: {unknown_ratio:.1%}, Current Iteration: {iteration}")
    
    if unknown_ratio > 0.40 and iteration < 2:
        print("Quality Check FAILED (>40% unknown risks). Initiating retry with relaxed RAG parameters...")
        # Increment iteration state for the next node
...
def save_graph_diagram(compiled_graph):
    ...
    try:
        ...
        print(f"Could not render and save graph diagram (likely missing pygraphviz or connection): {e}")

        with open(os.path.join(logs_dir, "workflow_graph.txt"), 'w') as f:
            f.write("START -> parse_document -> retrieve_context -> assess_risk -> (Quality Check Edge) -> flag_high_risk -> generate_recommendations -> compile_report -> END")
